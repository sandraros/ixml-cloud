...
CLASS lcl_ixml_unknown DEFINITION
  ...
  PROTECTED SECTION.
    ...
    DATA type TYPE lcl_ixml_factory=>tv_node_type.
...
CLASS lcl_ixml_node_collection DEFINITION
  ...
  PRIVATE SECTION.
    ...
    DATA document TYPE REF TO lcl_ixml_document.
    DATA node_ids TYPE lcl_ixml_factory=>tt_node_id.
...
CLASS lcl_ixml_node_iterator DEFINITION
  ...
  PRIVATE SECTION.
    ...
    DATA document        TYPE REF TO lcl_ixml_document.
    DATA node_list       TYPE REF TO lcl_ixml_node_list.
    DATA node_collection TYPE REF TO lcl_ixml_node_collection.
    DATA index           TYPE i.
...
CLASS lcl_ixml_factory IMPLEMENTATION.
  ...
  METHOD get_singleton.
    IF singleton IS NOT BOUND.
      singleton = NEW lcl_ixml_factory( ).
    ENDIF.
    rval = singleton.
  ENDMETHOD.
ENDCLASS.
...
CLASS lcl_ixml_unknown IMPLEMENTATION.
  METHOD zif_excel_ixml_unknown~query_interface.
    CASE iid.
      WHEN lcl_ixml_factory=>ixml_iid-element.
        IF type = zif_excel_ixml_node=>co_node_element.
          rval = me.
        ENDIF.
      WHEN lcl_ixml_factory=>ixml_iid-text.
        IF type = zif_excel_ixml_node=>co_node_text.
          rval = me.
        ENDIF.
      WHEN OTHERS.
